...
| CR 有效 | PR 首个有效评论者自动认领 CR，且只有该同学评论 `CR通过` 才算通过 |
...
PR 的 CR reviewer 由首个有效 PR conversation 评论者自动认领，不需要额外评论认领关键词。认领评论需要满足：
...
认领后，只有该 reviewer 本人继续在 PR 下评论：

```text
CR通过
```

才算 CR 通过。`CR通过` 评论需要满足：

- 评论者是已经自动认领该 PR 的 reviewer。
- 评论时间晚于 PR 最近一次 commit 时间。

如果 PR 在 CR 后又 push 了新 commit，原 `CR通过` 失效，需要原 reviewer 重新评论 `CR通过`。认领者不会因为后续 push 或其他同学评论而改变。

多人 CR 时，计分 reviewer 固定为首个有效 PR 评论者。同一 PR 只给一名 CR 同学计分。
...
- 实现首个有效 PR 评论者自动认领 CR，以及认领者 `CR通过` 判定。
- 接入 repo-guard 状态检查。
...
| 首个有效 PR 评论者评论问题，其他同学评论 `CR通过` | PR guard 不识别有效 CR |
| 首个有效 PR 评论者评论 `CR通过` | PR guard 识别有效 CR |
