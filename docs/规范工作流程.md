...
本文用于说明训练营期间的 GitHub 协作规则。所有同学按照这套流程认领任务、开发代码、提交 PR、接受维护者确认，并通过仓库工作流自动记录进度与积分。

> **阅读摘要**
>
> 每个任务都会以 Issue 的形式发布。同学在 Issue 下评论 `认领` 后，该任务会被锁定到自己名下；同一时间每位同学只能认领一个任务。开发完成后，从自己的 fork 向主仓库提交 PR，PR 必须关联 Issue。PR 满足基础条件后，需要组长/维护者在最新 commit 后评论 `确认合并`，才会自动合并并计分；如果 PR 24 小时内未合并，会被自动关闭，但任务仍归原认领者负责。
...
- PR 没有修改进度文件。
- Repo Guard、Codex、Copilot 已对最新 commit 给出自动评审反馈。
- 组长/维护者阅读自动评审反馈后评论 `确认合并`。
...
CR 鼓励但不强制。PR 是否合入以基础检查、Repo Guard/Codex/Copilot 反馈和组长/维护者 `确认合并` 为准。
...
才会让该 PR 记录一个有效 CR reviewer。其他同学即使评论 `CR通过`，也不会越过已认领的 reviewer。

如果 PR 后续又追加 commit，已认领 reviewer 之前评论过的 `CR通过` 仍然有效，不需要重新评论。

`CR通过` 只表示 CR 已记录，不会直接触发合并。工作流还需要组长/维护者在 PR 最新 commit 后评论：

```text
确认合并
```

如果 PR 后续又追加 commit，或自动评审在确认后才给出反馈，之前的 `确认合并` 会失效，需要组长/维护者重新确认。
...
如果原任务没有有效 CR，原 CR 扣分为 0。

如果 bug 修复 PR 没有有效 CR，修复开发者获得完整 `N` 分，修复 CR 分为 0。
