...
- `npm run subtitle:postprocess:real-model-smoke` 是手工执行的真实默认模型 smoke/benchmark：它使用当前公开 `ceilf6/code-tape-subtitle-postprocessor-onnx`、WASM `q8` 和代表性短字幕 fixture 跑一次 `SubtitlePostProcessor`，输出 `pipelineReadyDurationMs`、`generationDurationMs`、`totalDurationMs`、`jsonValid`、`segmentReferenceValid`、`chapterTimelineValid`、术语/章节命中率以及 `errorType`。该命令会下载或读取真实模型缓存，受网络、浏览器/Node WASM 后端和机器性能影响，不纳入 `quality:local` 硬门禁；涉及默认模型、prompt 或真实性能判断的 PR 应手工记录一次输出摘要。
