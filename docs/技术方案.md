...
- `npm run subtitle:postprocess:evaluate` 使用固定 fixture 评估“纠错并生成章节”输出，不加载真实浏览器 LLM，避免把网络、模型下载或 WebGPU/WASM 初始化耗时混入模型输出质量判断。
...
- 该命令输出 `overallEvaluationDurationMs` 作为本地评测入口从 fixture 样例评估到指标生成的整体耗时；`averageFixtureValidationDurationMs` 与 `maxFixtureValidationDurationMs` 只表示单条静态 fixture 解析和契约校验耗时。
- 这些耗时字段可作为后续 prompt、模型或 fallback 变更的可重复本地性能基线，但不能替代浏览器点击“纠错并生成章节”后的真实端到端等待时间。
- PR 涉及 AI 字幕后处理时，需要在描述或自检中记录一次该命令输出摘要，包括代表性样例通过率、JSON/segment/章节指标、`overallEvaluationDurationMs` 以及 fixture 校验耗时，作为 repo-guard、Codex 和人工审查的共同质量基线；真实模型下载、初始化、推理和 worker 通信耗时需要用单独的 runtime benchmark 记录。
